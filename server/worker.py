...
from app.core.llm import get_groq_client, get_openai_client
...
async def process_job(job: Dict[str, Any], judges_map: Dict[str, Dict[str, Any]]):
    ...
    await run_ai_judge_job(job, judges_map, supabase, groq_client, openai_client)
...
async def worker_loop():
    logging.info(
        "Worker starting; poll interval=%s, concurrency=%s, batch=%s",
        POLL_INTERVAL,
        CONCURRENCY,
        BATCH_SIZE,
    )
